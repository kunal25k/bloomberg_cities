async function drawLineChartVax() {
  ...
  const dataCases = await d3.json("data/linedata/casesVax.json")
  ...
  const wrapper = d3.select("#wrapperVAX")
  .append('center')
    .append("svg")
...
//   legendGroup.append("text").attr("x", 40).attr("y", 35).text("Covid Vaccine Approved").style("font-size", "10px").attr("alignment-baseline","middle")
//   legendGroup.append("text").attr("x", 40).attr("y", 50).text("Prevention").style("font-size", "10px").attr("alignment-baseline","middle")
...
//   legendGroup.append("text").attr("x", 40).attr("y", 65).text("Cases").style("font-size", "10px").attr("alignment-baseline","middle")  
  ...
  const xAxisGenerator = d3.axisBottom()
    ...
    .tickSize(0)
    .tickFormat(function(date){
      if (date > Date.parse("November 12, 2020") && date < Date.parse("December 12, 2020")){
        return d3.timeFormat("Dec 11, '20")(date);
      }
      else if (date > Date.parse("December 12, 2020") && date < Date.parse("January 12, 2021")){
        return d3.timeFormat('(Vaccines Approved)')(date);
      }
      else if (date > Date.parse("January 12, 2021") && date < Date.parse("November 12, 2021")){
        return d3.timeFormat('')(date);
      }
    })
