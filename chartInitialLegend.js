async function drawInitialLegend() {
    ...
    const wrapper = d3.select("#wrapperINITIALleg")
    .append('center')
    .append("svg")
...
if (window.innerWidth < 1025){
    ...
    const legendCircleCases = legendGroup.append("circle").attr("cx",125).attr("cy",18).attr("r", 3).style("fill", "red")
    const legendCircleCorrelation = legendGroup.append("circle").attr("cx",30).attr("cy",38).attr("r", 3).style("fill", "#B8ECB8")

    legendGroup.append("text").attr("x", 36).attr("y", 18).text("Number of Cases").style("font-size", "10px").attr("alignment-baseline","middle")
    legendGroup.append("text").attr("x", 131).attr("y", 18).text("Number of Prevention Policies").style("font-size", "10px").attr("alignment-baseline","red")  
    legendGroup.append("text").attr("x", 36).attr("y", 38).text("Time Period of Correlation between Prevention Policies and Cases").style("font-size", "10px").attr("alignment-baseline","middle")  
}
else {
    ...
    legendGroup.append("text").attr("x", 36).attr("y", 18).text("Number of Cases").style("font-size", "10px").attr("alignment-baseline","middle")
    legendGroup.append("text").attr("x", 131).attr("y", 18).text("Number of Prevention Policies").style("font-size", "10px").attr("alignment-baseline","middle")  
    legendGroup.append("text").attr("x", 36).attr("y", 38).text("Time Period of Correlation between Prevention Policies and Cases").style("font-size", "10px").attr("alignment-baseline","middle")  
}
