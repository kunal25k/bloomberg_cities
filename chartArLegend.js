async function drawARPALegend() {
    ...
    const wrapper = d3.select("#wrapperARPAleg")
    .append('center')
    .append("svg")
...
if (window.innerWidth < 1025){
    ...
    legendGroup.append("text").attr("x", 91).attr("y", 18).text("ARPA Policies").style("font-size", "10px").attr("alignment-baseline","middle")  
    legendGroup.append("text").attr("x", 36).attr("y", 38).text("ARPA Announcement Date").style("font-size", "10px").attr("alignment-baseline","middle")  
}
else {
    
    const legendCirclePrev = legendGroup.append("circle").attr("cx",30).attr("cy",18).attr("r", 3).style("fill", "blue")
    const legendCircleCases = legendGroup.append("circle").attr("cx",85).attr("cy",18).attr("r", 3).style("fill", "green")
    const legendCircleCorrelation = legendGroup.append("circle").attr("cx",175).attr("cy",18).attr("r", 3).style("fill", "red")
  
    legendGroup.append("text").attr("x", 36).attr("y", 18).text("Cases").style("font-size", "10px").attr("alignment-baseline","middle")
    legendGroup.append("text").attr("x", 91).attr("y", 18).text("ARPA Policies").style("font-size", "10px").attr("alignment-baseline","middle")  
    legendGroup.append("text").attr("x", 181).attr("y", 18).text("ARPA Announcement Date").style("font-size", "10px").attr("alignment-baseline","middle")  
}
