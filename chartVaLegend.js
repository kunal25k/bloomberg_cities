async function drawVaxLegend() {
    ...
    const wrapper = d3.select("#wrapperVAXleg")
    .append('center')
    .append("svg")
...
if (window.innerWidth < 1025){
    ...
    const legendCircleCases = legendGroup.append("circle").attr("cx",85).attr("cy",18).attr("r", 3).style("fill", "green")
    const legendCircleCorrelation = legendGroup.append("circle").attr("cx",30).attr("cy",38).attr("r", 3).style("fill", "red")

    legendGroup.append("text").attr("x", 36).attr("y", 18).text("Cases").style("font-size", "10px").attr("alignment-baseline","middle")
    legendGroup.append("text").attr("x", 91).attr("y", 18).text("Vaccination Policies").style("font-size", "10px").attr("alignment-baseline","middle")  
    legendGroup.append("text").attr("x", 36).attr("y", 38).text("First Vaccine Approval Date").style("font-size", "10px").attr("alignment-baseline","middle")  
}
else {
    
    const legendCirclePrev = legendGroup.append("circle").attr("cx",30).attr("cy",18).attr("r", 3).style("fill", "blue")
    const legendCircleCases = legendGroup.append("circle").attr("cx",85).attr("cy",18).attr("r", 3).style("fill", "green")
    const legendCircleCorrelation = legendGroup.append("circle").attr("cx",195).attr("cy",18).attr("r", 3).style("fill", "red")
  
    legendGroup.append("text").attr("x", 36).attr("y", 18).text("Cases").style("font-size", "10px").attr("alignment-baseline","middle")
    legendGroup.append("text").attr("x", 91).attr("y", 18).text("Vaccination Policies").style("font-size", "10px").attr("alignment-baseline","middle")  
    legendGroup.append("text").attr("x", 201).attr("y", 18).text("First Vaccine Approval Date").style("font-size", "10px").attr("alignment-baseline","middle")  
}
